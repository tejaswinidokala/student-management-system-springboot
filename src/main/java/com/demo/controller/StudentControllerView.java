...
//			@RequestParam(name = "sortDir", defaultValue = "asc", required = false)String sortDir,
		...
		if (name != null && !name.isBlank()) {
			dtos = service.filterStudentByContainingName(name);
		} else if (course != null && !course.isBlank()) {
			dtos = service.filterStudentByCourse(course);
		} else if (email != null && !email.isBlank() && mobile != null && !mobile.isBlank()) {
			StudentDto student = service.filterStudentByEmailAndMobile(email, mobile);
			dtos = student == null ? List.of() : List.of(student);
		} else if (email != null && !email.isBlank()) {
			StudentDto student = service.getStudentByEmail(email);
			dtos = student == null ? List.of() : List.of(student);
		} else if (mobile != null && !mobile.isBlank()) {
			dtos = service.filterStudentByEmailOrMobile("", mobile);
		} else {
			dtos = service.getStudents();
		}
		model.addAttribute("students", dtos);
		return "list_registration";
	}
